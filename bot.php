...
final class bot
{
    ...
    private function createBot($bot) {
        ...
        if (extension_loaded('memcache') && isset($bot->config['usememcache']) && $bot->config['usememcache'] == 1) {
            require ('./class_cache.php');
            $bot->memClass = new memClass((isset($bot->config['mchost']) ? $bot->config['mchost'] : '127.0.0.1'), (isset($bot->config['mcport']) ? $bot->config['mcport'] : 11211));
            $bot->parserClass->setMemClass($bot->memClass);
            $bot->ircClass->setMemClass($bot->memClass);
            $bot->dccClass->setMemClass($bot->memClass);
            $bot->timerClass->setMemClass($bot->memClass);
        }
    ...
    private function connectToDatabase($bot) {
        if (isset($bot->config['usedatabase'])) {
            ...
            if (!isset($bot->config['dbhost'])) $bot->config['dbhost'] = "localhost";
            if (!isset($bot->config['dbuser'])) $bot->config['dbuser'] = "root";
            if (!isset($bot->config['dbpass'])) $bot->config['dbpass'] = "";
            if (!isset($bot->config['db'])) $bot->config['db'] = "test";
            if (!isset($bot->config['dbprefix'])) $bot->config['dbprefix'] = "";
            if (!isset($bot->config['dbport'])) {
                $bot->db = new $dbType($bot->config['dbhost'], $bot->config['db'], $bot->config['dbuser'], $bot->config['dbpass'], $bot->config['dbprefix']);
            } 
            else {
                $bot->db = new $dbType($bot->config['dbhost'], $bot->config['db'], $bot->config['dbuser'], $bot->config['dbpass'], $bot->config['dbprefix'], $bot->config['dbport']);
            }
