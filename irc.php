...
class irc
{
    ...
    private $memClass = null;
    
    //Socket Vars
    private $sockInt;
    
    //old pre 2.1.2 method
    private $conn;
    ...
    function __construct() {
        ...
        $this->stats = array('BYTESUP' => 0, 'BYTESDOWN' => 0,);
        $this->pid = getmypid();
        return;
    }
    ...
    public function setMemClass($class) {
        $this->memClass = $class;
    }
    ...
    private function changeMember($channel, $oldNick, $newNick, $ident, $newStatus, $action, $newHost = "") {
        $channel = $this->myStrToLower($channel);
        $ident = trim($ident);
            ...
            if ($type == BY_MASK) {
                ...
                if ($newStatus == "b") {
                    if ($action == "+") {
                        ...
                        $chanPtr->banList[] = array('HOST' => $ban, 'TIME' => $time,);
                    } 
                    else {
    ...
    public function hostMasksMatch($mask1, $mask2) {
        ...
        $nick = false;
        $host = false;
        if ($maskA['ident'] == $maskB['ident'] || $maskA['ident'] == "*" || $maskB['ident'] == "*") {
            $ident = true;
        }
        if ($maskA['nick'] == $maskB['nick'] || $maskA['nick'] == "*" || $maskB['nick'] == "*") {
            $nick = true;
        }
        if ($maskA['host'] == $maskB['host'] || $maskA['host'] == "*" || $maskB['host'] == "*") {
            $host = true;
        }
        if ($host && $nick && $ident) {
            return true;
    ...
    private function register() {
        ...
        if ($this->getClientConf('serverpassword') != "") $login_string = "PASS " . $this->getClientConf('serverpassword') . "\r\n" . $login_string;
    ...
    private function readInput() {
        ...
        if ($this->socketClass->hasLine($this->sockInt)) {
            ...
            $this->lVars = array('from' => $parts[0], 'fromNick' => substr($parts[0], 0, $offset1), 'fromIdent' => substr($parts[0], $offset2, $offset4), 'fromHost' => substr($parts[0], $offset3), 'cmd' => $parts[1], 'to' => $parts[2], 'text' => substr($line, $offset5), 'params' => trim($params), 'raw' => ":" . $line,);
    ...
    private function parseServerMsgs($cmd) {
        switch ($cmd) {
            ...
            case 396:
                if ($this->getClientConf('operpass') != "" && !defined('OPER_SENT')) {
                    $this->pushBefore($this->clientFormat("OPER " . $this->getClientConf('opername') . " " . $this->getClientConf('operpass')));
                    define('OPER_SENT', 1);
                }
                break;
    ...
    public function parseModes($modeString) {
        ...
        while (trim($modes) != "") {
            ...
            $returnModes[] = array('ACTION' => $action, 'MODE' => $thisMode, 'EXTRA' => $extra, 'TYPE' => $type,);
        }
        return $returnModes;
    }
    ...
    private function parseMsgs() {
        switch ($this->lVars['cmd']) {
            ...
            case "MODE":
                ...
                if ($channel == $this->myStrToLower($this->nick)) break;
                ...
                if ($this->myStrToLower($this->lVars['fromNick']) == "nickserv") {
                    if (strpos($this->myStrToLower($this->lVars['text']), "identify") !== false) {
                        if ($this->getClientConf('password') != "") {
                            ...
                            $this->pushBefore($this->clientFormat("MODE " . $this->nick . " +B"));
                        }
                    }
                    if (strpos($this->myStrToLower($this->lVars['text']), "recognized") !== false) {
                    }
                }
                break;
    ...
    private function formatting($text) {
        preg_match_all('/\[c\](\d{1,2})(?:,(\d{1,2}))?/i', $text, $firstmatches);
        foreach ($firstmatches[1] as $key => $value) {
            $firstmatches[1][$key] = sprintf("%02u", $value);
        }
        foreach ($firstmatches[2] as $key => $value) {
            $firstmatches[2][$key] = sprintf("%02u", $value);
        }
        foreach ($firstmatches[0] as $key => $value) {
            $text = str_replace($value, chr(3) . $firstmatches[1][$key] . (preg_match('/,/', $value) ? "," . $firstmatches[2][$key] : ""), $text);
        }
        preg_match_all("/(\\x03\d{1,2}(?:,\d{1,2})?)( +)([^\W])/", $text, $matches);
        $count = count($matches[0]);
        $i = 0;
        while ($i < $count) {
            $text = str_replace($matches[0][$i], $matches[1][$i] . $matches[3][$i], $text);
            $i++;
        }
        $font = array('[c]' => chr(3), '[n]' => chr(15), '[b]' => chr(2), '[u]' => chr(31), '[r]' => chr(22));
        $text = strtr($text, $font);
    ...
    function privMsg($who, $msg, $queue = 1) {
        $msg = $this->formatting($msg);
        $text = array('USER' => $who, 'TEXT' => 'PRIVMSG ' . $who . ' :' . $msg);
        if ($queue) {
            $this->pushAfter($text);
        } 
        else {
    ...
    public function action($who, $msg, $queue = 1) {
        $msg = $this->formatting($msg);
        $text = array('USER' => $who, 'TEXT' => 'PRIVMSG ' . $who . ' :' . chr(1) . 'ACTION ' . $msg . chr(1));
        if ($queue) {
            $this->pushAfter($text);
        } 
        else {
    ...
    public function notice($who, $msg, $queue = 1) {
        preg_match_all('/\[c\](\d{1,2})(?:,(\d{1,2}))?/i', $msg, $firstmatches);
        foreach ($firstmatches[1] as $key => $value) {
            $firstmatches[1][$key] = sprintf("%02u", $value);
        }
        foreach ($firstmatches[2] as $key => $value) {
        ...
        foreach ($firstmatches[0] as $key => $value) {
            $msg = str_replace($value, "\x03" . $firstmatches[1][$key] . (preg_match('/,/', $value) ? "," . $firstmatches[2][$key] : ""), $msg);
        }
        preg_match_all("/(\\x03\d{1,2}(?:,\d{1,2})?)( +)([^\W])/", $msg, $matches);
        $count = count($matches[0]);
        $i = 0;
        while ($i < $count) {
            $msg = str_replace($matches[0][$i], $matches[1][$i] . $matches[3][$i], $msg);
            $i++;
        }
        $font = array('[c]' => "\x03", '[n]' => "\x0f", '[b]' => "\x02", '[u]' => "\x1f", '[r]' => "\x16");
        $msg = strtr($msg, $font);
        $text = array('USER' => $who, 'TEXT' => 'NOTICE ' . $who . ' :' . $msg);
        if ($queue) {
            $this->pushAfter($text);
        } 
        else {
