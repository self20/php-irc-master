    ...
    public function loadFuncs($file) {
        ...
        if (isset($this->cmdList['file'])) {
            foreach ($this->cmdList['file'] AS $file => $data) {
                ...
                $this->cmdList['file'][$file]['class']->__setClasses($this->ircClass, $this->dccClass, $this->timerClass, $this, $this->socketClass, $this->db, $this->memClass);
    ...
    */
    ...
    public static function createLine($rawLine) {
        ...
        if ($lineCount < 1) {
            ...
            while (($arg = array_shift($rawLineArray)) !== NULL)
            
            // NULL fixed contributed by cortex, 05/01/05
            {
                if (trim($arg) == "") {
                    continue;
                }
            ...
            case "005":
                if ($this->ircClass->getServerConf("NETWORK") != "") {
                    ...
                    if (strpos($line['params'], "NETWORK") !== false) {
                        ...
                        $this->dccClass->dccInform("Network has been identified as " . $this->ircClass->getServerConf("NETWORK") . "(" . $line['from'] . ")");
                    }
                }
                break;
